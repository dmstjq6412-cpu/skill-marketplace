...
export default function SearchBar({ value, onChange, placeholder = 'Search skills...' }) {
  return (
    <div className="relative group">
      <svg
        className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400 dark:text-slate-500 group-focus-within:text-primary-500 dark:group-focus-within:text-primary-400 transition-colors duration-150 w-[18px] h-[18px]"
      ...
      <input
        ...
        className="w-full pl-11 pr-4 py-3.5 bg-white dark:bg-[#111218] border border-slate-200 dark:border-slate-800 rounded-2xl text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-600 shadow-sm dark:shadow-none focus:outline-none focus:ring-2 focus:ring-primary-600/50 focus:border-primary-400 dark:focus:border-primary-500 transition-all duration-150 font-sans"
      />
      {value && (
        <button
          onClick={() => onChange('')}
